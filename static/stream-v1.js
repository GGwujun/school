...
(function () {
	var Provider, aFilters = [], nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sCellFileTemplate = '<b class="stream-uploading-ico"></b>' +
			...
			'	<span class="stream-cell-info speed">\u901f\u5ea6\uff1a<em class="stream-speed"></em></span>' +
			'	<span class="stream-cell-info uploaded">\u5df2\u4e0a\u4f20\uff1a<em class="stream-uploaded"></em></span>' +
			'	<span class="stream-cell-info  remain-time">\u5269\u4f59\u65f6\u95f4\uff1a<em class="stream-remain-time"></em></span>' +
			'</div>',
	...
	Main.prototype = {
		...
		upload: function (index) {
			...
			var frmUploadURL = this.get('host') ? this.get('host') + this.get("frmUploadURL") : this.get("frmUploadURL");
			var uploadURL = this.get('host') ? this.get('host') + this.get("uploadURL") : this.get("uploadURL");
			...
			var tokenUrl = fAddVars(vars, this.get("host") ? this.get("host") + this.get("tokenURL") : this.get("tokenURL")) + "&" + fGetRandom();
