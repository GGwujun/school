<template>
    <div class="list">
        ...
        <el-table border style="width: 100%" align='center' :data="user_list" @selection-change='onSelectionChange'>
            <el-table-column label='住址' type="expand">
            ...
            <el-table-column prop="username" label="姓名" align="center" width="150" :sortable="true">
            </el-table-column>
            <el-table-column prop="email" label="邮箱" align="center" :sortable="true">
            </el-table-column>
            <el-table-column prop="sex" label="性别" align="center" width="130" sortable="true" :formatter="formatterSex" :filters="sex_filters.list" :filter-method="filterSex" :filter-multiple="sex_filters.multiple">
            </el-table-column>
            <el-table-column prop="create_time" label="创建时间" align="center" width="180" :formatter="formatterDate" :sortable="true">
            </el-table-column>
            <el-table-column prop="status" label="状态" align="center" width="130" :sortable="true" :formatter="formatterStatus" :filters="status_filters.list" :filter-method="filterStatus" :filter-multiple="status_filters.multiple">
            </el-table-column>
        ...
        <el-dialog title="用户信息" v-model="dialog.show" size="tiny">
            <el-form style="margin:20px;width:60%;min-width:100%" label-width="100px" :model="dialog.user_info">
                ...
                <el-form-item label="生日">
                    {{formatterYear(dialog.user_info.birthday)}}
                </el-form-item>
                <el-form-item class="edit-form" label='地址'>
                ...
                <el-form-item label="创建时间">
                    {{formatterDate(dialog.user_info.create_time)}}
                </el-form-item>
                <el-form-item label="最后更新时间">
                    {{formatterDate(dialog.user_info.update_time)}}
                </el-form-item>
            </el-form>
        ...
        <el-dialog title="设置权限" v-model="dialog_access.show" size="small">
            <el-form style="margin:20px;width:60%;min-width:100%" label-width="100px" :model="dialog_access.userinfo">
                <el-form-item class='edit-form' label="邮箱" prop='email'>
                ...
                <el-form-item class='edit-form' label="前端页面">
                    <!-- CheckBox选项列表 -->
                ...
                <el-form-item class='edit-form'>
                    <el-tree class="filter-tree" show-checkbox default-expand-all node-key="path" :data="accesss" :props="defaultProps" :filter-node-method="filterNode" @check-change='checkChange' @current-change='currentChange' @node-click='nodeClick' ref="accesss">
                    </el-tree>
                </el-form-item>
